...
public sealed class CreateDepartmentUseCase : ICreateDepartmentUseCase
{
    ...
    public CreateDepartmentUseCase(IDepartmentRepository repository) => _repository = repository;

    public async Task<Department> ExecuteAsync(CreateDepartmentInput input, CancellationToken ct = default)
    {
        if (string.IsNullOrWhiteSpace(input.Name))
            throw new ValidationException("Name", "Tên phòng ban không được để trống.");

        var existing = await _repository.GetAllAsync(ct);
        if (existing.Any(d => d.Name.Equals(input.Name.Trim(), StringComparison.OrdinalIgnoreCase)))
