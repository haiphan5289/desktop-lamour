...
public sealed class UpdateWarehouseSettingUseCase : IUpdateWarehouseSettingUseCase
{
    private readonly IWarehouseSettingRepository _repository;
    public UpdateWarehouseSettingUseCase(IWarehouseSettingRepository repository) => _repository = repository;

    public async Task<WarehouseSetting> ExecuteAsync(UpdateWarehouseSettingInput input, CancellationToken ct = default)
    {
        ...
        if (existing.Any(w => w.Id != input.Id && w.Code.Equals(input.Code.Trim(), StringComparison.OrdinalIgnoreCase)))
