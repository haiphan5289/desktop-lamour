...
public sealed class UpdateAccountSettingUseCase : IUpdateAccountSettingUseCase
{
    private readonly IAccountSettingRepository _repository;
    public UpdateAccountSettingUseCase(IAccountSettingRepository repository) => _repository = repository;

    public async Task<AccountSetting> ExecuteAsync(UpdateAccountSettingInput input, CancellationToken ct = default)
    {
        ...
        if (existing.Any(a => a.Id != input.Id && a.Code.Equals(input.Code.Trim(), StringComparison.OrdinalIgnoreCase)))
