...
- **Acceptance criteria:**
  - [x] Hiển thị danh sách 8 cột: Mã KH, Tên, Địa chỉ, Tỉnh, Nhóm KH, Mã số thuế, Điện thoại, Sale
...
| ViewModel | `ViewModels/CustomerFormViewModel.cs` | Form state, `LoadAsync` (employees + next code) |
...
CustomerFormWindow (OnContentRendered)
  → CustomerFormViewModel.LoadAsync()
    → [parallel-like] IGetEmployeesUseCase.ExecuteAsync()  → Employees dropdown populated
    → (Add mode only) ICustomerService.GetNextCodeAsync()  → Code field hiển thị
    → (Edit mode) SelectedEmployee = match by SaleCare name (case-insensitive)
```
...
- [`Views/CustomerFormWindow.xaml`](../Views/CustomerFormWindow.xaml) — Form: Code (disabled), Name*, Phone, Address, Province, CustomerGroup, TaxCode, **NV chăm sóc** (`AppSearchableComboBox` — dropdown nhân viên)
- [`Views/CustomerFormWindow.xaml.cs`](../Views/CustomerFormWindow.xaml.cs) — `OnContentRendered` → `LoadAsync()` (với try/catch)
...
- [`ViewModels/CustomerFormViewModel.cs`](../ViewModels/CustomerFormViewModel.cs) — `LoadAsync()` (employees + next code), `SaveCommand`, `CancelCommand`; inject `IGetEmployeesUseCase` + `ILogger`; `SelectedEmployee` (`ISearchableItem?`) thay cho `SaleCare` string

### Domain
- [`Domain/Models/Customer.cs`](../Domain/Models/Customer.cs) — `Id`, `Code`, `Name`, `Address`, `Province`, `CustomerGroup`, `TaxCode`, `Phone`, `SaleCare`
- [`Domain/UseCases/CreateCustomerInput.cs`](../Domain/UseCases/CreateCustomerInput.cs) — record: `Name`, `Phone`, `Address`, `Province`, `CustomerGroup`, `TaxCode`, `SaleCare`
- [`Domain/UseCases/UpdateCustomerInput.cs`](../Domain/UseCases/UpdateCustomerInput.cs) — record: `Id`, `Name`, `Phone`, `Address`, `Province`, `CustomerGroup`, `TaxCode`, `SaleCare`
...
| `GetNextCode` API lỗi | Log warning, Code field trống | ✅ (logged) |
| Load employees API lỗi | Log warning, dropdown trống — vẫn có thể nhập thủ công nếu cần | ✅ (logged) |
...
- `CustomerFormViewModel` inject `IGetEmployeesUseCase` (load employees cho dropdown NV chăm sóc) + `ILogger`
- `SaleCare` không còn là text field tự nhập — lưu `SelectedEmployee?.Name` khi save; khi edit auto-select NV theo tên (case-insensitive)
- `LoadAsync()` thay `LoadNextCodeAsync()`: load employees trước, rồi next-code (chỉ Add mode)

---

*Generated by `/ct-ai-document` on 2026-04-25 — Updated 2026-04-26: thêm TotalCustomersText badge — Updated 2026-05-23: thêm cột Sale (`SaleCare`) vào DataGrid, form, domain model, inputs, và DTOs*
*Updated 2026-06-11: field "Sale" đổi từ AppTextField → AppSearchableComboBox dropdown nhân viên; `LoadNextCodeAsync` → `LoadAsync` (load employees); inject `IGetEmployeesUseCase` + `ILogger`; fix `async void OnContentRendered`*
