...
- **Acceptance criteria:**
  ...
  - [x] Tile "📊 Báo cáo" trên màn hình menu "Bán hàng" (`SalesView`, cùng cấp "Chứng từ bán hàng"/"Hàng bán bị trả lại"/"Phiếu cọc"/"Báo cáo trừ cọc"; 2026-08-22: đã thử chuyển xuống toolbar `SalesOrderListView` rồi chuyển ngay lại đây trong ngày theo yêu cầu — vị trí cuối cùng vẫn là tile trên `SalesView` như từ 2026-07-18) → popup filter (Mặt hàng/Nhân viên/Khách hàng/Đơn vị tính/Nhóm VTHH/Kỳ báo cáo/Từ-Đến ngày) → trang báo cáo riêng với DataGrid + xuất Excel + in (2026-07-16, mở rộng 2026-07-18)
  ...
  - [x] "Sổ chi tiết bán hàng" — mỗi cột DataGrid có 1 filter riêng nhúng thẳng trong header (không phải popup, luôn hiện sẵn): 6 cột text (Số chứng từ/Khách hàng/Nhân viên/Mã hàng/Tên hàng/ĐVT) có 1 ô nhập lọc contains; cột Ngày hạch toán + 7 cột số (Số lượng/Đơn giá/CK%/Thành tiền/Thuế suất/Tiền thuế/Tổng cộng) có 1 `ComboBox` toán tử (=, ≠, <, ≤, >, ≥) đặt cạnh 1 ô nhập giá trị/DatePicker, cả 2 nằm ngay trong header, không cần bấm mở gì; ảnh hưởng tới cả tổng cộng footer và Print/Xuất Excel/Email/Zalo; nút "🧹 Xóa điều kiện" reset toàn bộ 14 filter cùng lúc (2026-08-22)
  - [x] `SalesView` (màn hình hub "Bán hàng", vào từ Trang chủ → 🛒 Bán hàng, 5 tile: Chứng từ bán hàng/Hàng bán bị trả lại/**Báo cáo**/Phiếu cọc/Báo cáo trừ cọc) — tile "📊 Báo cáo" (khôi phục 2026-08-26, sau khi từng bị xoá 2026-08-22) mở `SalesOrderReportFilterWindow` → `SalesOrderReportView`, trang riêng, đổi được "Thống kê theo" (7 kiểu) + group/subtotal + In/Xuất Excel/Email/Zalo/"Chọn tham số" đổi lại filter tại chỗ — xem mục "Báo cáo bán hàng" bên dưới. Panel "📊 Báo cáo bán hàng theo Mặt hàng" từng nhúng thẳng trên `SalesView` (2026-08-22–2026-08-26, cộng dồn cố định theo Mặt hàng) **đã bị gỡ bỏ hoàn toàn** — trùng chức năng với tile sau khi tile được khôi phục.
...
| Báo cáo là filter-popup + trang riêng (2026-07-16) | Tile "📊 Báo cáo" trên `SalesView` (2026-07-18: chuyển từ toolbar `SalesOrderListView` lên đây; 2026-08-22: thử chuyển xuống lại toolbar rồi đảo ngược ngay trong ngày — vị trí cuối cùng vẫn ở `SalesView`) mở `SalesOrderReportFilterWindow` (chỉ chứa filter, `ShowDialog`) — không phải trang báo cáo hiển thị luôn; submit xong mới điều hướng sang `SalesOrderReportView` (trang riêng, không phải popup) |
...
| Sổ chi tiết bán hàng — lọc theo từng cột, nhúng thẳng trong header (mới 2026-08-22, sửa lại từ bản popup ban đầu) | `SalesOrderReportDetailView` mỗi cột có 1 control filter đặt ngay trong `DataGridTextColumn.HeaderTemplate`, KHÔNG dùng `Popup` — luôn hiện sẵn, không cần bấm mở. 6 cột text (Số chứng từ/Khách hàng/Nhân viên/Mã hàng/Tên hàng/ĐVT) chỉ có 1 `AppTextField` bind 2-way lên property `Filter*` (string) trên ViewModel, lọc = Contains (case-insensitive) trên đúng chuỗi hiển thị ở cell. Cột Ngày hạch toán + 7 cột số dùng `DateColumnFilter`/`NumericColumnFilter` (`Domain/Models/ColumnFilterModels.cs`) — 1 `ComboBox` nhỏ (chỉ hiện ký hiệu qua `FilterOperatorSymbolConverter`, tooltip hiện nhãn đầy đủ qua `FilterOperatorLabelConverter`) đặt cạnh 1 `AppTextField`/`DatePicker` nhập giá trị, cả 2 nằm trong 1 `Grid` 2 cột ngay dưới label tiêu đề — không có control nào ẩn sau click |
| Sổ chi tiết bán hàng — vì sao không dùng `Popup` | Bản đầu tiên dùng `ToggleButton` mở `Popup` chứa checklist/operator để tiết kiệm không gian (giống ảnh MISA tĩnh), nhưng user muốn nội dung lọc luôn hiện "kế bên icon" — không cần bấm. Đổi lại: bỏ hẳn checklist đa-giá-trị cho cột text (không đủ chỗ hiện inline), quay về Contains-textbox đơn giản; cột ngày/số giữ được operator+value vì `ComboBox` + `AppTextField`/`DatePicker` đủ nhỏ để nhúng cạnh nhau trong 1 header ~90-140px (đã nới rộng vài cột: Ngày hạch toán 105→135, Số lượng 75→95, Đơn giá 100→120, CK% 90→110, Thành tiền 110→130, Thuế suất 80→100, Tiền thuế 100→120, Tổng cộng 120→140, ĐVT 60→70) |
| Sổ chi tiết bán hàng — filter là client-side, không gọi lại API | `LoadAsync` load 1 lần vào `_allItems` (list gốc, không đổi khi lọc); `ApplyFilters()` derive `Items` (collection bind DataGrid) = `_allItems.Where(MatchesAllFilters)` mỗi khi 1 trong 14 filter đổi (6 property string qua `OnFilter*Changed` partial hook, 8 filter object qua callback `Changed` gán trong `WireColumnFilters()`) — không có round-trip BE nào cho việc lọc theo cột |
| Sổ chi tiết bán hàng — filter ảnh hưởng tổng + export | Vì `Print`/`ExportExcelCommand`/`SendEmailCommand`/`SendZaloCommand` (`BuildWorkbook`/`BuildReportDocument`) và tổng footer (`TotalQuantity`/`TotalAmount`/`TotalTaxAmount`/`TotalGrandTotal`) đều đọc từ `Items` (không phải `_allItems`), nên tất cả tự động chỉ tính/xuất trên các dòng đang khớp filter — không cần sửa gì thêm ở các command đó |
| Sổ chi tiết bán hàng — nút "Xóa điều kiện" | `ClearFiltersCommand` set 6 property `Filter*` (text) về rỗng, reset `Operator`/`Value`/`ValueText` về mặc định (`Equal`/`null` cho ngày, `LessOrEqual`/rỗng cho số) cho 8 filter ngày/số — mỗi thay đổi tự trigger `ApplyFilters()` riêng (gọi lại nhiều lần nhưng vô hại, dataset nhỏ) |
...
- [`ViewModels/SalesOrderListViewModel.cs`](../ViewModels/SalesOrderListViewModel.cs) — Commands: `AddSalesOrder`, `EditSalesOrder`, `HoldSalesOrder`, `DeleteSalesOrder`, `LoadSalesOrders`, `GoBack` (`OpenReport` không nằm ở đây — xem `SalesViewModel`); `EditSalesOrderAsync` hiện confirm dialog trước khi mở form
- [`Views/SalesView.xaml`](../Views/SalesView.xaml) / [`ViewModels/SalesViewModel.cs`](../ViewModels/SalesViewModel.cs) — Menu "Bán hàng" cấp cha; `OpenReportCommand` (tile "📊 Báo cáo") inject `Func<SalesOrderReportFilterWindow>` — cùng factory đã đăng ký sẵn trong DI, không cần đăng ký thêm
...
### Báo cáo bán hàng (2026-07-16, di chuyển lên `SalesView` 2026-07-18; 2026-08-22: thử chuyển xuống `SalesOrderListView` rồi đảo ngược lại `SalesView` trong ngày theo yêu cầu — vị trí cuối cùng: tile trên `SalesView`)

**UX flow (hiện tại):** ô "📊 Báo cáo" là 1 tile trên `SalesView` (màn hình menu "Bán hàng", vào từ Trang chủ → 🛒 Bán hàng) → mở `SalesOrderReportFilterWindow` (popup, `ShowDialog`, chỉ chứa filter — không hiển thị kết quả trong popup) → chọn Mặt hàng/Nhân viên/Khách hàng (optional, `AppSearchableComboBox` tái dùng `IGetProductsUseCase`/`IGetEmployeesUseCase`/`IGetCustomersUseCase` đã có sẵn) + Từ ngày/Đến ngày (optional) → "Đồng ý" → popup đóng → điều hướng sang `SalesOrderReportView` (trang riêng, không phải popup) hiển thị DataGrid dòng chi tiết khớp filter + totals footer + nút In/Xuất Excel.

> **2026-07-18:** `OpenReportCommand` được di chuyển từ `SalesOrderListViewModel` (toolbar của `SalesOrderListView`, bên trong "Chứng từ bán hàng") sang `SalesViewModel` (tile trên màn hình menu "Bán hàng" cấp cha) — theo yêu cầu để nút Báo cáo dễ thấy hơn, không cần vào sâu "Chứng từ bán hàng" trước.

> **2026-08-22 (chuyển xuống rồi đảo ngược lại, cùng ngày):** Ban đầu chuyển `OpenReportCommand` xuống `SalesOrderListViewModel`/toolbar `SalesOrderListView` theo 1 yêu cầu đọc nhầm ý ("Bán hàng" xuất hiện 2 lần trong câu, tưởng là 2 màn khác nhau) — user phản hồi ngay sau khi thấy `SalesView` (hub, vào từ Trang chủ) không còn hiện Báo cáo, yêu cầu trả về đúng vị trí cũ. Chuyển lại `OpenReportCommand` (+ `Func<SalesOrderReportFilterWindow>` factory) về `SalesViewModel`/tile trên `SalesView`, xoá nút khỏi toolbar `SalesOrderListView` — kết quả cuối cùng y hệt trạng thái từ 2026-07-18, không đổi gì so với trước khi có toàn bộ đợt sửa 2026-08-22 này.

```csharp
// SalesViewModel (giữ nguyên từ 2026-07-18 — không đổi sau đợt sửa/đảo ngược 2026-08-22)
[RelayCommand]
...
*Updated 2026-08-22 (loạt fix + feature mới ở popup `SalesOrderWindow` — cùng phiên với migrate "Phiếu nhập kho" sang `AppSearchableComboBox`, xem `Warehouse/docs/warehouse.md`):*
  - **Migrate cột "Mã hàng"/"Tên hàng" sang `AppSearchableComboBox`**: trước đó dùng `ComboBox` thường + code-behind chắp vá trong `SalesOrderWindow.xaml.cs` (`OnProductCellTextChanged` bắt `TextChangedEvent` bubble từ `LinesDataGrid` để tự gọi `FilterProductsByCode`/`FilterProductsByName` + mở dropdown thủ công, `LinesDataGrid_PreparingCellForEdit` vá lỗi WPF mất ký tự đầu khi gõ, `RestoreTypedTextDeferred` chống Text/con trỏ nhảy loạn khi dùng bộ gõ tiếng Việt Unikey — ~80 dòng, xem lịch sử trước 2026-08-22 nếu cần đối chiếu). Migrate thẳng sang `controls:AppSearchableComboBox` (đã ổn định từ khi dùng cho "Phiếu nhập kho"/"Đối tượng"/"NV bán hàng") — xoá sạch code-behind vá lỗi trên, xoá `FilterProductsByCode`/`FilterProductsByName`/diff-based `RefreshProducts` khỏi `SalesOrderViewModel` (không cần nữa vì `AppSearchableComboBox` tự lọc nội bộ theo Code/Name/Phone mỗi khi gõ — xem `PopulateFiltered` trong control); `ResetProductFilter()` đơn giản hoá thành `Products.Clear()` + `foreach Add()` thẳng. "Trừ cọc" (`DepositProductPickerItem` trộn đầu `_allProducts`) không đổi hành vi gì — vẫn hoạt động nguyên vẹn qua migrate này.
  - **Fix "Hàng KM" không tick được** (`DataGridCheckBoxColumn` → `DataGridTemplateColumn`): user báo tick "Hàng KM" nhưng Thành tiền không về 0, Tổng tiền hàng không đổi — xác nhận qua số liệu (Tổng tiền hàng vẫn cộng cả dòng đã tick) rằng `IsPromotion` chưa từng thật sự đổi thành `true`, dù checkbox NHÌN đã tick. Nguyên nhân: lỗi kinh điển của `DataGridCheckBoxColumn` — click đầu tiên trên 1 cell chưa phải "current cell" chỉ chọn cell, không forward click tới CheckBox để toggle+commit. Fix: đổi hẳn cột "Hàng KM" sang `DataGridTemplateColumn` với `CheckBox` đặt thẳng trong `CellTemplate` (`IsChecked="{Binding IsPromotion, Mode=TwoWay, UpdateSourceTrigger=PropertyChanged}"`) — checkbox nhúng kiểu này luôn tương tác được ngay, không cần "vào edit mode" trước, nên 1 click là toggle+commit ngay, không còn cần cell đã "current".
  - **Fix "Thuế suất" hiện `800%` thay vì `8%`**: `ConverterParameter="0\%"` (dấu `\` để .NET hiểu `%` là ký tự literal, không nhân giá trị lên 100 lần khi format) viết trong cú pháp `{Binding TaxRate, Converter=..., ConverterParameter=0\%}` rút gọn — dấu `\` bị nuốt mất bởi ngữ pháp escape của MarkupExtension trước khi tới `BlankPreserveConverter`, khiến converter nhận đúng `"0%"` (format percent chuẩn .NET, LUÔN nhân 100) → `TaxRate=8` hiện `"800%"`. Fix: viết lại `Binding` dạng mở rộng `<DataGridTextColumn.Binding><Binding Path="TaxRate" Converter="{StaticResource BlankPreserveConverter}" ConverterParameter="0\%"/></DataGridTextColumn.Binding>` — ở dạng attribute XML thường (không qua cú pháp `{...}`), `\` không có ý nghĩa escape đặc biệt nên `"0\%"` giữ nguyên đúng 3 ký tự cần thiết.
  - **Fix banner lỗi không tự ẩn**: banner đỏ (ví dụ "Các sản phẩm không đủ tồn kho") chỉ được `HasError = false` ở đầu `SaveAsync` — sửa/xoá dòng gây lỗi xong mà chưa bấm "Ghi sổ" lại thì banner vẫn còn nguyên, gây hiểu nhầm chưa sửa được. Fix: `OnLinesOrTotalsChanged()` (hook có sẵn, chạy mỗi khi `Lines` thêm/bớt/1 field đổi) giờ tự `HasError = false; ErrorMessage = string.Empty;` mỗi lần — banner biến mất ngay khi user chỉnh lại form, không cần đợi lần Ghi sổ kế tiếp.
  - **Fix cột phụ thuộc (ĐVT/Đơn giá/TK...) không tự hiện ngay sau khi chọn sản phẩm**: chọn xong sản phẩm (click item trong `AppSearchableComboBox`) — ViewModel đã set đúng `Unit`/`UnitPrice`/`ReceivableAccount`/... ngay trong `SelectedProduct` setter, nhưng `DataGrid` không tự vẽ lại các cell KHÁC cùng dòng khi dòng đó còn ở trạng thái "edit" của riêng cell Mã/Tên hàng. Fix: `AppSearchableComboBox` thêm `RoutedEvent SelectionCommittedEvent` (bubble, raise trong `SelectItem()` — chỉ bắn khi user CLICK CHỌN THẬT, không bắn khi gõ tay tự do); `SalesOrderWindow` constructor `AddHandler` event này trên `LinesDataGrid`, gọi `LinesDataGrid.CommitEdit(DataGridEditingUnit.Row, true)` qua `Dispatcher.BeginInvoke(DispatcherPriority.ContextIdle, ...)` (trì hoãn để tránh đụng state máy edit nội bộ DataGrid ngay trong lúc nó còn xử lý click vừa rồi) — chọn xong là toàn bộ dòng hiện đủ ngay, không cần Tab/click ra ngoài.
  - **Trừ cọc — fix mất khoản trừ khi mở lại chứng từ + tự gợi ý số tiền**: `PopulateFormFromCurrentAsync` trước đó chỉ nạp lại `SalesOrderLine` thật, KHÔNG nạp lại `DepositDeduction` đã ghi sổ (entity riêng, không phải 1 `SalesOrderLine`) — mở lại 1 chứng từ đã Trừ cọc sẽ hiện lại "Tổng thanh toán (gồm thuế)" TRƯỚC khi trừ (sai), in lại từ màn này cũng sai theo. Fix: gọi thêm `IGetDepositDeductionsUseCase.ExecuteAsync(salesOrderId: CurrentOrder.Id)` (API sẵn có, không cần sửa BE) trong `PopulateFormFromCurrentAsync`, dựng lại 1 dòng "Trừ cọc" cho mỗi `DepositDeduction` tìm được với `IsLocked = true` (property mới trên `SalesOrderLineItem`) — hiện đúng số nhưng KHOÁ không cho sửa/xoá (`RemoveLine` chặn xoá dòng `IsLocked`; `SalesOrderWindow.xaml` thêm style `DisabledWhenLocked` áp cho 3 cột Mã/Tên hàng/Thành tiền — 3 cột duy nhất KHÔNG bị disable sẵn bởi style `DisabledForDepositRow` cũ) — tránh gọi lại `CreateDepositDeductionUseCase` lúc Lưu và tạo bản ghi trùng lặp. Kèm UX cải thiện theo yêu cầu tiếp: chọn "Trừ cọc" trong dropdown giờ tự điền sẵn "Thành tiền" gợi ý = `min(số dư còn lại của cọc, TotalPayment + TotalTaxAmount hiện tại)` — logic gộp vào `AttachLineHandlers` (helper mới, thay cho lambda trùng lặp trước đây giữa `AddLine()` và loop nạp lại chứng từ), bắt trên sự kiện đổi `LinkedDeposit` (KHÔNG phải `SelectedProduct` — property đó bắn `PropertyChanged` NGAY khi gán, TRƯỚC khi nhánh `DepositProductPickerItem` trong setter kịp gán `LinkedDeposit`, nên bắt ở `SelectedProduct` sẽ luôn đọc giá trị `LinkedDeposit` cũ/null).
  - **Feature mới — tách "Khách hàng" thành "Mã số KH"/"Tên Khách hàng" tuỳ chỉnh + "Địa chỉ" tuỳ chỉnh + ẩn "Tham chiếu"**: theo yêu cầu, field "Khách hàng" tách 2 ô cùng hàng — "Mã số KH" giữ nguyên `AppSearchableComboBox` cũ (set `SelectedCustomer`/`CustomerId`, không đổi); "Tên Khách hàng" là control MỚI tự viết `Shared/Controls/AppSuggestTextBox` — **cố tình KHÔNG sửa `AppSearchableComboBox` chung** (đang dùng ở rất nhiều màn khác — Kho, NV bán hàng, Đối tượng...) vì hành vi cần khác nhau về gốc: `AppSearchableComboBox.OnSearchLostFocus` LUÔN trả `SearchBox.Text` về `SelectedItem.DisplayText` nếu rời ô mà text gõ không khớp item nào (đúng ý cho 1 field bắt buộc phải là 1 item thật) — còn "Tên Khách hàng" cần NGƯỢC LẠI: gợi ý theo tên khi gõ (tái dùng `SearchableItemDropdownTextConverter` + pattern Popup/ListBox y hệt) nhưng GIỮ NGUYÊN đúng text tự do user gõ dù không khớp ai, không tự trả về tên cũ. Chọn 1 gợi ý trong `AppSuggestTextBox` set `Text = item.Name` + gọi `SuggestionPickedCommand` (mới, tương tự `AddCommand` có sẵn ở `AppSearchableComboBox`) — `SalesOrderViewModel.CustomerNameSuggestionPickedCommand` dùng để đồng bộ lại `SelectedCustomer` khi user chọn gợi ý từ đúng field này. "Địa chỉ" (field mới, `AppTextField` thường — không cần gợi ý vì không phải dữ liệu để search) đặt TRÊN "Diễn giải" trong layout (Grid row 0 Khách hàng → row 2 Địa chỉ MỚI → row 4 Diễn giải, các row sau dồn xuống 2 bậc). Cả "Tên Khách hàng"/"Địa chỉ" tự điền từ `Customer.Name`/`Customer.Address` khi chọn 1 khách hàng thật (qua `OnSelectedCustomerChanged`) nhưng cho gõ đè tự do sau đó (không đổi `CustomerId`/công nợ). Đổi "Tên Khách hàng" — từ bất kỳ nguồn nào (chọn khách hàng HAY gõ tay) — luôn đè lại "Diễn giải" = `"Bán hàng {tên}"` (giữ đúng hành vi auto-fill cũ, chỉ đổi nguồn từ `Customer.Name` cứng sang field mới `CustomerNameText`). "Tham chiếu" ẩn khỏi UI popup này (`Grid Visibility="Collapsed"`) theo yêu cầu — binding `Reference` và field ở BE giữ nguyên hoàn toàn, không xoá gì (chỉ ẩn hiển thị, không mất dữ liệu chứng từ cũ). **BE**: thêm `SalesOrder.CustomerNameOverride`/`CustomerAddressOverride` (2 migration, xem `be-window-lamour/.../Sales/docs/sales.md`) — chỉ gửi lên khi text THỰC SỰ khác giá trị thật đang chọn (`ResolveCustomerNameOverride()`/`ResolveCustomerAddressOverride()`, so sánh với `SelectedCustomer.Name`/`.Address`) để tên/địa chỉ tự đổi theo nếu khách hàng được sửa lại sau này, không "đóng băng" giá trị cũ 1 cách không cố ý. In hoá đơn: `SalesOrderPrintWindow` dùng `order.CustomerAddress` (đã resolve override từ BE, hoặc từ `BuildPreviewOrderDto` nếu chưa lưu) thay `SelectedCustomer.Address` trực tiếp; dòng "Điện thoại" bị ẨN HOÀN TOÀN (không chỉ để trống sau dấu `:`) khi `CustomerNameOverride` có giá trị — số điện thoại thật không còn khớp với tên đang hiển thị trên hoá đơn nên không in ra, tránh gây nhầm số của khách khác; áp dụng đồng nhất cho cả in ngay sau khi Ghi sổ và in preview chưa lưu.
  - Cả BE lẫn WPF build 0 lỗi mỗi bước. Chưa test thật trên UTM (chỉ build được, không chạy được WPF UI từ máy Mac) — cần user tự verify trên UTM trước khi coi là xong hẳn. Known follow-up: `SalesReturnWindow` vẫn giữ pattern `ComboBox` + code-behind vá lỗi y hệt Sales trước khi sửa lần này — candidate migrate tương tự nếu gặp lại đúng bug "gõ tên không hiện list"; 6 màn khác đang dùng `DataGridCheckBoxColumn` (`SupplierListView`, `ProductListView`, `EmployeeListView`, `WarehouseSettingListView`, `SalesOrderReportFilterWindow`, `BulkCustomerReceiptSearchWindow`) có thể mang lỗi tương tự bug "Hàng KM" nhưng chưa audit/fix.*
*Updated 2026-08-22 (chuyển nút "📊 Báo cáo" xuống toolbar `SalesOrderListView`, rồi đảo ngược lại `SalesView` NGAY TRONG NGÀY — xem chi tiết đầy đủ ở mục "Báo cáo bán hàng" phía trên): request gốc dùng "Bán hàng" 2 lần trong 1 câu ("move logic Báo cáo ở màn hình 'Bán hàng' ra màn hình 'Bán hàng'") — hiểu lầm ban đầu là chuyển ngược 2026-07-18 (tile `SalesView` → toolbar `SalesOrderListView`, bên trong "Chứng từ bán hàng"). Sau khi build xong, user vào Trang chủ → 🛒 Bán hàng và không thấy Báo cáo ở màn hub (`SalesView`) nữa, phản hồi ngay: ý thật là muốn Báo cáo hiện NGAY Ở `SalesView` (màn vào thẳng từ Trang chủ), không phải ẩn sâu trong "Chứng từ bán hàng". Đảo ngược lại: `OpenReportCommand` + `Func<SalesOrderReportFilterWindow>` factory chuyển VỀ `SalesViewModel`/tile trên `SalesView`; xoá field/constructor param tương ứng khỏi `SalesOrderListViewModel`; xoá nút khỏi toolbar `SalesOrderListView.xaml`; tile "📊 Báo cáo" chèn lại đúng vị trí cũ trên `SalesView.xaml` (giữa "Hàng bán bị trả lại" và "Phiếu cọc"). Kết quả ròng: không đổi gì so với trước khi bắt đầu đợt sửa 2026-08-22 này. Bài học: khi 1 câu yêu cầu nhắc tên màn hình giống nhau 2 lần, nên hỏi lại/xác nhận cụ thể ĐÚNG 2 màn hình khác nhau nào trước khi build, thay vì đoán theo ngữ cảnh code. Không có BE change nào. WPF build 0 lỗi mỗi bước. Chưa test thật trên UTM.*

*Updated 2026-08-22 (panel "📊 Báo cáo theo Mặt hàng" nhúng thẳng trên `SalesOrderListView`): theo yêu cầu, khi vào "Chứng từ bán hàng" tự động hiện luôn 1 bảng tổng hợp bán hàng theo Mặt hàng, kỳ Đầu tháng đến hiện tại — không cần bấm nút "📊 Báo cáo" riêng nữa. Đã hỏi lại trước khi build vì đây là 1 khu vực UI mới, không phải chỉ wiring lại chỗ đặt nút như 2 request trước. Chốt: (1) nhúng panel thật (không auto-navigate sang trang báo cáo, không chỉ đổi filter mặc định của popup); (2) dữ liệu = bảng tổng hợp theo Mặt hàng, KHÔNG lọc thêm Nhân viên/Khách hàng/ĐVT/Nhóm VTHH; (3) đặt ngay dưới Toolbar actions, trên Filter bar, có nút thu gọn/mở rộng (mặc định mở); (4) tự load lại mỗi khi `LoadSalesOrdersAsync` chạy lại (Thêm/Sửa/Treo/Xóa/Lọc).
  - **`SalesOrderListViewModel`**: inject thêm `IGetSalesOrderSummaryReportUseCase` (đã đăng ký sẵn theo TYPE trong DI từ tính năng báo cáo tổng hợp 2026-07-18, không cần đăng ký thêm). Thêm `IsReportPanelExpanded` (mặc định `true`), `IsReportLoading`, `HasReportError`, `ReportErrorMessage`, `HasReportRows`, `ObservableCollection<ProductSalesSummaryRow> ReportRows`, `ToggleReportPanelCommand`. Thêm `LoadReportPanelAsync()` — gọi `_getSummaryReport.ExecuteAsync(null, null, null, null, null, startOfMonth, today, ct)` (không filter theo product/employee/customer/unit/category, chỉ khoảng ngày), `GroupBy(l => l.ProductId)` rồi `Sum` từng field số (`QuantitySold`/`SalesAmount`/`DiscountAmount`/`ReturnQuantity`/`ReturnValue`/`NetRevenue`) — gộp mọi khách hàng/nhân viên của cùng 1 sản phẩm thành 1 dòng duy nhất, giống đúng cách `SalesOrderReportViewModel` xử lý "Thống kê theo" = "Mặt hàng" (nhưng viết lại độc lập, không tái dùng `ReportDisplayRow`/`GroupingsByType` vì panel này không cần đổi "Thống kê theo" hay filter gì khác — chỉ 1 chế độ cố định). `LoadReportPanelAsync()` gọi ở CUỐI `LoadSalesOrdersAsync()` (sau khối `try/catch/finally` tải danh sách chứng từ) — 2 lần load độc lập nhau về mặt filter (list dùng `FilterFromDate`/`FilterToDate` do user chọn, panel LUÔN cố định đầu-tháng-đến-hôm-nay) nhưng CÙNG trigger — bất kỳ hành động nào gọi `LoadSalesOrdersCommand`/`LoadSalesOrdersAsync` (Thêm/Sửa/Treo/Xóa/Lọc/mở màn hình lần đầu qua `Loaded` event có sẵn trong `SalesOrderListView.xaml.cs`) đều tự kéo theo reload panel, không cần sửa gì thêm ở các command đó.
  - Thêm mới `Domain/Models/ProductSalesSummaryRow.cs` — model hiển thị 1 dòng/sản phẩm đã cộng dồn (`ProductCode`/`ProductName`/`Unit`/`QuantitySold`/`SalesAmount`/`DiscountAmount`/`ReturnQuantity`/`ReturnValue`/`NetRevenue`), tách riêng khỏi `SalesOrderSummaryLineItem` (vốn là 1 dòng/triple product-customer-employee CHƯA gộp) vì panel này luôn hiển thị Ở MỨC ĐÃ GỘP, không cần giữ `CustomerId`/`EmployeeId`.
  - **`SalesOrderListView.xaml`**: thêm 1 `RowDefinition Height="Auto"` mới (giữa Toolbar actions và Filter bar) — `AppLoadingOverlay` gốc đổi `Grid.RowSpan="4"` → `"5"`; Filter bar đổi `Grid.Row="2"` → `"3"`; Grid chứa DataGrid+footer đổi `Grid.Row="3"` → `"4"` (không đổi nội dung bên trong 2 khối này, chỉ dịch số hàng). Panel mới = 1 `Border` cùng style với Toolbar/Filter bar (`AppColor.BackgroundPrimary`, border mỏng, `CornerRadius="8"`) chứa: hàng tiêu đề (`AppLabel` + `AppButton.Tertiary.Medium` toggle — nội dung nút đổi qua 2 `TextBlock` "▲ Thu gọn"/"▼ Mở rộng" ẩn/hiện bằng `BoolToVisibilityConverter`/`InverseBoolToVisibilityConverter` có sẵn, không cần converter mới); nội dung thu gọn được (`Grid` với `Visibility` bind `IsReportPanelExpanded`) chứa `AppLoadingOverlay` riêng (`IsReportLoading`) + label lỗi (`HasReportError`) + label rỗng (`!HasReportRows`) + `DataGrid` nhỏ (`MaxHeight="240"`, `CanUserSortColumns="False"`, 9 cột: Mã hàng/Tên hàng/ĐVT/SL bán/Doanh số bán/Chiết khấu/SL trả lại/GT trả lại/Doanh thu thuần — cột cuối tô `AppColor.TextBrand` + `SemiBold` giống các cột "tổng" khác trong app).
  - Không có BE change nào — dùng nguyên `GET /api/v1/sales-orders/summary-report` đã có sẵn từ 2026-07-18. WPF build 0 lỗi. Chưa test thật trên UTM.*

*Updated 2026-08-22 (sửa lại vị trí panel "📊 Báo cáo theo Mặt hàng" — chuyển từ `SalesOrderListView` sang `SalesView`): entry ngay phía trên mô tả bản build đầu tiên đặt panel trên `SalesOrderListView` ("Chứng từ bán hàng") — hiểu nhầm câu request gốc `"Bán hàng" screen (Quản lý chứng từ"`. Sau khi build xong, user gửi screenshot màn hub `SalesView` (title "Bán hàng", subtitle **"Quản lý chứng từ bán hàng"** — chính subtitle này mới là thứ request gốc nhắc tới trong ngoặc, KHÔNG phải màn "Chứng từ bán hàng" riêng) và hỏi "màn hình này có thay đổi gì đâu" — xác nhận lại ý muốn: panel phải nằm ngay trên `SalesView` (hub), không phải sau khi bấm vào tile "Chứng từ bán hàng".
  - Gỡ toàn bộ phần đã thêm khỏi `SalesOrderListViewModel`/`SalesOrderListView.xaml` (field `_getSummaryReport`, 5 property `IsReportPanelExpanded`/`IsReportLoading`/`HasReportError`/`ReportErrorMessage`/`HasReportRows`, `ReportRows`, `ToggleReportPanelCommand`, `LoadReportPanelAsync()`, lời gọi cuối `LoadSalesOrdersAsync()`, `RowDefinition`/`Border` panel trong XAML) — trả `SalesOrderListView` về đúng layout gốc (4 hàng: header/toolbar/filter/DataGrid).
  - Chuyển y hệt logic đó sang `SalesViewModel`/`SalesView.xaml`: inject thêm `IGetSalesOrderSummaryReportUseCase` (đã đăng ký sẵn theo TYPE trong DI, không cần đăng ký thêm); property + `ToggleReportPanelCommand`/`LoadReportPanelAsync()` y hệt bản trước (cùng công thức `GroupBy(l => l.ProductId)` + `Sum` từng field). Khác biệt duy nhất về cơ chế trigger: `SalesOrderListView` có nhiều hành động mutate data (Thêm/Sửa/Treo/Xóa/Lọc) nên gắn `LoadReportPanelAsync()` vào cuối `LoadSalesOrdersAsync()` là hợp lý; `SalesView` (hub) không có hành động nào mutate data — panel chỉ cần load 1 LẦN DUY NHẤT khi màn hình mở, nên gọi qua `Loaded` event trong `SalesView.xaml.cs` (`Loaded += async (_, _) => await viewModel.LoadReportPanelCommand.ExecuteAsync(null);`), giống hệt pattern `SalesOrderListView.xaml.cs` đã dùng cho `LoadSalesOrdersCommand`.
  - `SalesView.xaml`: panel chèn ngay sau `WrapPanel` chứa 5 tile (trong cùng `StackPanel` cuộn được của `ScrollViewer`, không phải `Grid.Row` riêng như bản `SalesOrderListView` — vì `SalesView` chỉ có 2 row Auto/*, không có toolbar/filter bar để chèn giữa) — cùng style `Border` (nền trắng, viền mỏng, `CornerRadius="8"`) và cùng bộ 9 cột DataGrid y hệt bản trước.
  - Không đổi `Domain/Models/ProductSalesSummaryRow.cs` — model này dùng chung, không thuộc riêng view nào. Không có BE change nào. WPF build 0 lỗi mỗi bước. Chưa test thật trên UTM. Bài học (lặp lại từ 2 entry trước): parse kỹ phần trong ngoặc của FEATURE_REQUEST khi nó dùng để chỉ định CHÍNH XÁC màn hình nào — đối chiếu với subtitle/title thật trên UI thay vì suy luận theo tên file code, để tránh build nhầm màn.*

*Updated 2026-08-22 (đưa toàn bộ field filter từ popup `SalesOrderReportFilterWindow` lên thẳng panel trên `SalesView`, xoá tile "📊 Báo cáo"): theo yêu cầu tiếp theo sau khi panel "Báo cáo theo Mặt hàng" đã đúng chỗ (`SalesView`) — user muốn panel đó có ĐẦY ĐỦ field lọc như popup cũ thay vì cố định "đầu tháng đến hiện tại", và xác nhận rõ: xoá luôn tile "📊 Báo cáo" + không dùng flow popup/`SalesOrderReportView` nữa — panel trên `SalesView` là cách DUY NHẤT xem báo cáo bán hàng.
  - **`SalesViewModel`**: xoá `_reportFilterWindowFactory`/`OpenReportCommand`. Thêm 3 dependency mới `IGetProductsUseCase`/`IGetEmployeesUseCase`/`IGetCustomersUseCase` (đã đăng ký sẵn theo TYPE trong DI, dùng chung ở rất nhiều màn khác — không cần đăng ký thêm). Copy nguyên logic từ `SalesOrderReportFilterViewModel` (đã ngừng dùng, xem bên dưới) vào `SalesViewModel`: `SelectedEmployee`/`SelectedCustomer`/`SelectedUnit`/`SelectedCategory`/`SelectedPeriod`/`FromDate`/`ToDate`/`AreAllProductsSelected`, collection `Employees`/`Customers`/`Units`/`Categories`/`ProductItems` (`ObservableCollection<ProductCheckItem>`), `LoadLookupsAsync()` (load song song Product/Employee/Customer qua `Task.WhenAll`, derive `Units`/`Categories` distinct từ Product), `ApplyProductFilter()` (lọc `ProductItems` theo Unit/Category đã chọn), `ApplyPeriodPreset(period)` (9 preset tính `FromDate`/`ToDate`, y hệt bản gốc). Thêm `InitializeReportPanelCommand` (gọi `LoadLookupsAsync` rồi `LoadReportPanelAsync`, dùng làm entry point duy nhất từ `Loaded` event thay vì gọi thẳng `LoadReportPanelCommand` như bản trước) và `ClearReportFiltersCommand` (reset tất cả field lọc về mặc định RỒI tự gọi lại `LoadReportPanelAsync` — khác popup cũ chỉ reset field mà không tự submit).
  - `LoadReportPanelAsync()` sửa lại — không còn hardcode `null, null, null, null, null, startOfMonth, today` nữa mà build tham số thật từ field lọc: `productIds` = `ProductItems.Where(IsSelected).Select(Id)` (rỗng → `null`, nghĩa là không lọc theo mặt hàng cụ thể nào — giữ nguyên hành vi "để trống = tất cả" của popup cũ), `SelectedEmployee?.Id`, `SelectedCustomer?.Id`, `SelectedUnit`/`SelectedCategory` (map `"Tất cả"` → `null` qua `IsAnyOption()`), `FromDate`/`ToDate`. Vẫn giữ nguyên bước `GroupBy(l => l.ProductId)` + `Sum` từng field — dimension hiển thị VẪN CỐ ĐỊNH là "Mặt hàng" (không thêm "Thống kê theo" như popup cũ có — quyết định trước đó của panel này, không đổi lại lần này vì câu hỏi chỉ xoay quanh "field filter", không phải dimension hiển thị).
  - **`SalesView.xaml`**: xoá hẳn tile "Báo cáo" (Border thứ 3 trong `WrapPanel`, giữa "Hàng bán bị trả lại" và "Phiếu cọc") — `WrapPanel` giờ còn 4 tile: Chứng từ bán hàng/Hàng bán bị trả lại/Phiếu cọc/Báo cáo trừ cọc. Panel đổi tiêu đề "📊 Báo cáo theo Mặt hàng (Đầu tháng đến hiện tại)" → "📊 Báo cáo bán hàng theo Mặt hàng" (không còn cố định kỳ báo cáo trong tiêu đề nữa vì giờ chọn được). Thêm y hệt layout filter form của `SalesOrderReportFilterWindow.xaml` cũ (2 `Grid` label+input hàng ngang: Kỳ báo cáo full-width; Từ/Đến `DatePicker`; Đơn vị tính/Nhóm VTHH `ComboBox`; Nhân viên/Khách hàng `AppSearchableComboBox`) + checklist Mặt hàng (`DataGrid` với `DataGridCheckBoxColumn` + "Chọn tất cả", `MaxHeight="160"` để không chiếm quá nhiều chỗ trên 1 panel nhúng) + 2 nút "🔍 Lọc" (`LoadReportPanelCommand`)/"Xóa điều kiện" (`ClearReportFiltersCommand`) — đặt NGAY TRÊN khu vực k queết quả (trước đây ẩn trong popup riêng, giờ luôn hiện sẵn khi panel mở rộng).
  - `SalesView.xaml.cs`: `Loaded` event đổi từ gọi `LoadReportPanelCommand` sang `InitializeReportPanelCommand` (cần load lookup Employee/Customer/Product/Unit/Category trước khi user có thể chọn field nào, không chỉ load thẳng báo cáo như bản trước).
  - **Chưa xoá file**: `SalesOrderReportFilterWindow.xaml(.cs)`, `SalesOrderReportFilterViewModel.cs`, `SalesOrderReportView.xaml(.cs)`, `SalesOrderReportViewModel.cs`, `Domain/Models/ReportDisplayRow.cs`, `Domain/Models/SalesOrderReportTypes.cs`, route `NavigationRoutes.SalesOrders.Report`, và các dòng DI liên quan trong `HomeServiceCollectionExtensions.cs` VẪN CÒN TRONG CODE nhưng không còn ai gọi tới (không có navigation entry point nào trỏ vào `SalesOrderReportView` nữa sau khi xoá `OpenReportCommand`) — cố ý CHƯA xoá vật lý các file này trong lần sửa này vì đây là hành động khó đảo ngược (mất luôn: 7 kiểu "Thống kê theo" với group/subtotal, Print, Xuất Excel, nút "Chọn tham số" mở lại filter tại chỗ) — cần user xác nhận thêm 1 lần nữa nếu muốn xoá hẳn các file này.
  - Known issue kế thừa (không phải lỗi mới): cột checklist "Mặt hàng" dùng `DataGridCheckBoxColumn` y hệt bản popup cũ — đã biết bug "click đầu tiên trên cell chưa phải current cell không toggle được checkbox" (ghi nhận ở entry 2026-08-22 "Fix Hàng KM không tick được", liệt `SalesOrderReportFilterWindow` vào danh sách "chưa audit/fix") — bug này giờ theo luôn vào panel `SalesView` vì dùng chung code, chưa fix trong lần sửa này.
  - Không có BE change nào. WPF build 0 lỗi mỗi bước. Chưa test thật trên UTM.*

*Updated 2026-08-22 (ẩn section "Mặt hàng" khỏi panel báo cáo trên `SalesView`): theo yêu cầu, ẩn checklist "Mặt hàng" (label + "Chọn tất cả" + `DataGrid` checklist) khỏi UI — bọc trong 1 `StackPanel Visibility="Collapsed"`, KHÔNG xoá field/logic liên quan trong `SalesViewModel` (`ProductItems`/`AreAllProductsSelected`/`ApplyProductFilter`) để dễ mở lại sau này nếu cần (chỉ cần bỏ `Visibility="Collapsed"`). `LoadReportPanelAsync` không đổi — vì không còn cách nào tick chọn sản phẩm, `productIds` gửi lên BE sẽ luôn `null` (không lọc theo mặt hàng cụ thể), tương đương hành vi "để trống = tất cả" vẫn đúng. Đơn vị tính/Nhóm VTHH vẫn cần load `Product` list để derive distinct values nên `LoadLookupsAsync` không đổi. Không có BE change nào. WPF build 0 lỗi.*

*Updated 2026-08-22 (gọn lại filter form trên panel `SalesView`): theo yêu cầu "sắp xếp UI lại cho gọn" — bản trước dùng 2 `Grid` xếp nhiều hàng (Kỳ báo cáo chiếm nguyên 1 hàng dù chỉ cần 1 ô, Từ/Đến 1 hàng, ĐVT/Nhóm VTHH 1 hàng, Nhân viên/Khách hàng 1 hàng riêng) — tốn chiều cao và có khoảng trắng thừa cạnh "Kỳ báo cáo". Gộp cả 6 field (Kỳ báo cáo/Từ/Đến/Đơn vị tính/Nhóm VTHH/Nhân viên/Khách hàng) vào 1 `WrapPanel` duy nhất (label+control nối tiếp nhau, tự xuống hàng nếu hẹp) — khớp đúng phong cách "Filter bar" đã có sẵn ở `SalesOrderListView.xaml` (label `Margin` phải 8px, control `Height="36"`, cách field kế tiếp 20px). Không đổi field/binding/logic nào trong `SalesViewModel` — thuần sắp xếp lại layout. WPF build 0 lỗi.*

*Updated 2026-08-22 (thu nhỏ 4 tile "Nghiệp vụ bán hàng" trên `SalesView`): theo yêu cầu — 4 tile (Chứng từ bán hàng/Hàng bán bị trả lại/Phiếu cọc/Báo cáo trừ cọc) đổi `Width="176" Height="160"` → `Width="130" Height="112"`, icon `FontSize="40"` → `"26"`, margin dưới label `"0,12,0,0"` → `"0,8,0,0"`, khoảng cách giữa các tile `Margin="0,0,16,16"` → `"0,0,12,12"` — tất cả co theo cùng tỷ lệ cho gọn. Không đổi command/binding nào. WPF build 0 lỗi.*

*Updated 2026-08-22 (2 nút "← Quay lại"/"🏠 Trang chủ" cùng hàng với title "Bán hàng" trên `SalesView`): trước đó 2 nút nằm ở 1 hàng ngang riêng, phía TRÊN title "Bán hàng" (2 hàng tách biệt). Theo yêu cầu, gộp cả 2 nút VÀ title vào chung 1 `StackPanel Orientation="Horizontal"` (đều `VerticalAlignment="Center"`) — subtitle "Quản lý chứng từ bán hàng" vẫn nằm ở hàng riêng bên dưới như cũ. Không đổi command nào. WPF build 0 lỗi.*

*Updated 2026-08-22 (giảm padding header + đổi 4 tile "Nghiệp vụ bán hàng" sang layout icon+text ngang, thu nhỏ thêm): theo yêu cầu — (1) header: `Margin="24,32,24,0"` → `"24,16,24,0"` (giảm top padding), `StackPanel` nội dung cuộn `Margin="24,24,24,24"` → `"24,12,24,16"`, label "Nghiệp vụ bán hàng" `Margin` dưới `12`→`8`; (2) 4 tile đổi từ layout dọc (icon `FontSize="26"` trên, text dưới, `Border 130x112`) sang layout ngang (icon `FontSize="20"` bên trái + text bên phải cùng hàng, `VerticalAlignment="Center"` cho cả 2, `Border 210x52` — rộng hơn để chứa text ngang nhưng THẤP hơn nhiều vì không còn xếp chồng dọc) — margin giữa các tile `"0,0,12,12"` → `"0,0,12,10"`. Không đổi command nào. WPF build 0 lỗi.*

*Updated 2026-08-22 (xoá subtitle "Quản lý chứng từ bán hàng" + fix filter form vỡ dòng khi wrap trên `SalesView`): (1) xoá hẳn `AppLabel` subtitle dưới title "Bán hàng" theo yêu cầu. (2) Fix bug layout: `WrapPanel` filter form trước đó chứa 14 item RỜI (7 cặp label+control liên tiếp) — khi bề rộng không đủ, `WrapPanel` ngắt dòng giữa CHỪNG 1 cặp (ví dụ nhãn "Nhân viên:" ở cuối 1 hàng, ô `AppSearchableComboBox` của nó rớt xuống đầu hàng kế tiếp một mình, nhìn như UI bị vỡ/mất). Fix: bọc mỗi cặp label+control vào riêng 1 `StackPanel Orientation="Horizontal"` (margin phải chuyển từ control sang StackPanel bọc ngoài) — giờ `WrapPanel` chỉ ngắt dòng GIỮA các field, không bao giờ tách label khỏi control của chính nó. Không đổi binding/logic nào. WPF build 0 lỗi.*

*Updated 2026-08-22 (fix "Nhân viên"/"Khách hàng" bị cắt mất đáy + tinh chỉnh spacing filter form trên `SalesView`): root cause — `AppSearchableComboBox` (`Shared/Controls/AppSearchableComboBox.xaml`) có `FieldBorder` bên trong hardcode `Height="44"` (không bind ra ngoài), nhưng bản trước set `Height="36"` trên chính thẻ `<controls:AppSearchableComboBox>` ở `SalesView.xaml` — UserControl bị ép co xuống 36px trong khi nội dung bên trong (Border cao 44px) không co theo, dẫn tới bị crop mất ~8px đáy. Fix: đổi TẤT CẢ control trong filter form (2 `ComboBox` Kỳ báo cáo/Đơn vị tính/Nhóm VTHH, 2 `DatePicker` Từ/Đến, 2 `AppSearchableComboBox` Nhân viên/Khách hàng) sang `Height="44"` đồng bộ — vừa hết bug crop vừa đều hàng, không còn control nào cao/thấp lệch nhau trong cùng 1 hàng. Nhân tiện tinh chỉnh spacing cho "đẹp" hơn theo yêu cầu: khoảng cách giữa các field 20→24px, khoảng cách label→control 8→10px, khoảng cách xuống hàng khi wrap 8→12px, `Width` một số ô nhích lên chút (Kỳ báo cáo 150→160, Từ/Đến 125→130, ĐVT/Nhóm VTHH 130→135, Nhân viên/Khách hàng 190→200) cho thoáng hơn. Không đổi binding/logic nào. WPF build 0 lỗi.*

*Updated 2026-08-22 (thêm double-click drill-down cho panel báo cáo trên `SalesView`): theo yêu cầu, double-click 1 dòng trong bảng kết quả "Báo cáo bán hàng theo Mặt hàng" → mở "Sổ chi tiết bán hàng" (`SalesOrderReportDetailView`) lọc đúng theo `ProductId` của dòng đó, kế thừa nguyên `SelectedEmployee`/`SelectedCustomer`/`SelectedUnit`/`SelectedCategory`/`FromDate`/`ToDate` đang lọc trên panel. Tái dùng nguyên hạ tầng `SalesOrderDetailFilter`/`NavigationRoutes.SalesOrders.ReportDetail`/`SalesOrderReportDetailView` đã có sẵn từ tính năng drill-down cũ của `SalesOrderReportView` (2026-07-31) — trang đó giờ không còn ai NAVIGATE TỚI (`OpenReportCommand` đã xoá), nhưng `SalesOrderReportDetailView` vẫn là 1 ĐIỂM ĐẾN hợp lệ, độc lập với nơi nào điều hướng tới nó.
  - Thêm `ProductId` (int) vào `Domain/Models/ProductSalesSummaryRow.cs` — trước đó model chỉ có `ProductCode`/`ProductName` (đủ để hiển thị) nhưng thiếu Id thật để build filter khi drill-down; `LoadReportPanelAsync` set `ProductId = g.Key` (key của `GroupBy(l => l.ProductId)`, vốn đã tính sẵn, chỉ chưa gán ra model).
  - `SalesViewModel` thêm `DrillDownReportRowCommand` — build `SalesOrderDetailFilter { Title, ProductId, EmployeeId, CustomerId, Unit, Category, FromDate, ToDate }` từ đúng field lọc hiện tại của panel rồi `NavigateTo(NavigationRoutes.SalesOrders.ReportDetail, filter)`.
  - `SalesView.xaml`: `DataGrid` kết quả đặt tên `x:Name="ReportRowsGrid"`, thêm `SelectionMode="Single"` + `MouseDoubleClick="ReportRowsGrid_MouseDoubleClick"`. `SalesView.xaml.cs` thêm handler y hệt pattern `SalesOrderReportDetailView.xaml.cs` (`DetailGrid_MouseDoubleClick`) — đọc `SelectedItem`, check `CanExecute`, gọi `Execute`.
  - Không có BE change nào — `SalesOrderReportDetailView`/`IGetSalesOrderReportUseCase` dùng nguyên, không sửa gì. WPF build 0 lỗi.*

*Updated 2026-08-22 (tách bảng kết quả "Báo cáo bán hàng theo Mặt hàng" ra thành 1 section/card riêng trên `SalesView`): theo yêu cầu "để mình có scroll" — trước đó filter form + bảng kết quả nằm chung 1 `Border` card, bảng kết quả bị gò trong `MaxHeight="240"` chật hẹp cùng chỗ với form lọc. Tách thành 2 `Border` card riêng biệt, cùng style (nền trắng, viền mỏng, `CornerRadius="8"`, cách nhau `Margin="0,12,0,0"`):
  - Card 1: tiêu đề + nút Thu gọn/Mở rộng + filter form (Kỳ báo cáo/Từ/Đến/ĐVT/Nhóm VTHH/Nhân viên/Khách hàng) + Mặt hàng (ẩn) + nút Lọc/Xóa điều kiện — giữ nguyên nội dung, chỉ đóng `Border` sớm hơn (ngay sau nút Lọc/Xóa điều kiện).
  - Card 2 (MỚI): chỉ chứa khu vực kết quả — loading overlay/label lỗi/label rỗng/`DataGrid` (`ReportRowsGrid`). `Visibility` của CẢ CARD (không chỉ nội dung bên trong như trước) bind `IsReportPanelExpanded` — bấm "Thu gọn" ở Card 1 giờ ẩn LUÔN cả Card 2, không để lại card trắng trống phía dưới.
  - `DataGrid.MaxHeight` tăng từ `240` → `420` — giờ có nguyên 1 card riêng cho bảng nên tăng vùng hiển thị trước khi cần cuộn nội bộ (`ScrollViewer.VerticalScrollBarVisibility="Auto"` giữ nguyên).
  - Không đổi binding/command/logic nào trong `SalesViewModel`. WPF build 0 lỗi.*

*Updated 2026-08-22 (chuyển `ColumnFilterModels.cs` ra `Shared/Models`, dùng chung cho nhiều module): theo yêu cầu nhân bản tính năng filter theo cột của "Sổ chi tiết bán hàng" sang các màn "Sổ chi tiết"-style khác (Accounting/Warehouse/Deposits) — để tránh các module đó phải phụ thuộc chéo vào `Sales.Domain.Models`, chuyển `FilterOperator`/`FilterOperatorSymbols`/`NumericColumnFilter`/`DateColumnFilter` từ `Features/HomePage/Sales/Domain/Models/ColumnFilterModels.cs` sang `Shared/Models/ColumnFilterModels.cs` (namespace đổi từ `DesktopLamour.Features.HomePage.Sales.Domain.Models` → `DesktopLamour.Shared.Models`). `Shared/Converters/FilterOperatorSymbolConverter.cs`/`FilterOperatorLabelConverter.cs` cập nhật `using` theo namespace mới. `SalesOrderReportDetailView.xaml` (`xmlns:models`) và `SalesOrderReportDetailViewModel.cs` (`using`) cập nhật tương ứng — hành vi của "Sổ chi tiết bán hàng" hoàn toàn không đổi, thuần di chuyển vị trí class. WPF build 0 lỗi. Xem `Warehouse/docs/warehouse.md` cho 2 màn Warehouse mới áp dụng pattern này; Accounting (Sổ Kế Toán Chi Tiết Quỹ Tiền Mặt) và Deposits (Báo cáo đơn trừ cọc) cũng đã áp dụng nhưng chưa có doc riêng để ghi lại.

*Updated 2026-08-22 (fix cuộn ngang cho "Sổ Kế Toán Chi Tiết Quỹ Tiền Mặt" và "Báo cáo đơn trừ cọc" — ghi ở đây vì cùng đợt fix với ColumnFilterModels): cả 2 màn có cột `Width="*"` (Accounting: "Diễn giải"; Deposits: "Khách hàng" + "Diễn giải") gây lỗi WPF DataGrid quen thuộc — cột star bóp vừa khít viewport thay vì cuộn ngang. Đổi sang fixed width (Accounting "Diễn giải"→250; Deposits "Khách hàng"→170, "Diễn giải"→220), thêm tường minh `ScrollViewer.HorizontalScrollBarVisibility="Auto"`/`VerticalScrollBarVisibility="Auto"` trên `DataGrid` của cả 2 màn. Xem `Warehouse/docs/warehouse.md` cho 2 màn Warehouse cùng đợt fix này. WPF build 0 lỗi.*

*Updated 2026-08-26 (khôi phục lại tile "📊 Báo cáo" trên `SalesView`, mở lại `SalesOrderReportView`): theo yêu cầu — user muốn "báo cáo" hoạt động như tile "Phiếu cọc" (bấm vào mở hẳn 1 trang/popup riêng), không phải chỉ panel nhúng cộng-dồn-theo-Mặt-hàng cố định đang có. `SalesOrderReportFilterWindow`/`SalesOrderReportViewModel`/`SalesOrderReportView` chưa từng bị xoá vật lý (chỉ mất lối vào từ 2026-08-22, xem 3 entry phía trên) nên chỉ cần nối lại, không phải viết mới.
  - **`SalesViewModel`**: thêm lại `Func<SalesOrderReportFilterWindow> _reportFilterWindowFactory` (inject qua constructor — factory đã đăng ký sẵn trong DI từ trước, không cần đăng ký thêm) + `OpenReportCommand` — y hệt implementation gốc 2026-07-18 (`window.ShowDialog()` → `true` thì `window.BuildFilter()` → `NavigateTo(NavigationRoutes.SalesOrders.Report, filter)`).
  - **`SalesView.xaml`**: thêm lại tile "📊 Báo cáo" (cùng style/kích thước `210x52` với 4 tile kia), chèn giữa "Hàng bán bị trả lại" và "Phiếu cọc" — đúng vị trí lịch sử trước khi bị xoá.
  - Không có BE change nào. WPF build 0 lỗi. Chưa test thật trên UTM.*

*Updated 2026-08-26 (gỡ hẳn panel "📊 Báo cáo bán hàng theo Mặt hàng" nhúng trên `SalesView`): theo yêu cầu tiếp ngay sau khi khôi phục tile "📊 Báo cáo" ở trên — panel nhúng giờ trùng chức năng với tile (cả 2 đều xem báo cáo bán hàng), user chọn giữ tile (đầy đủ 7 kiểu "Thống kê theo" + In/Xuất Excel/Email/Zalo) và bỏ hẳn panel (chỉ cộng dồn cố định theo Mặt hàng). Ngược lại hoàn toàn với quyết định 2026-08-22 (lúc đó chọn giữ panel, xoá tile) — lần này đảo lại đúng theo yêu cầu mới nhất.
  - **`SalesView.xaml`**: xoá cả 2 card "Report panel" (form lọc Kỳ báo cáo/Từ-Đến/ĐVT/Nhóm VTHH/NV/KH + checklist Mặt hàng ẩn + nút Lọc/Xóa điều kiện) và "Report results" (`DataGrid ReportRowsGrid` 7 cột) — `WrapPanel` chứa 5 tile giờ là phần tử cuối cùng trong `StackPanel` cuộn được.
  - **`SalesView.xaml.cs`**: bỏ `Loaded` event handler (từng gọi `InitializeReportPanelCommand`) và `ReportRowsGrid_MouseDoubleClick` — file giờ chỉ còn constructor set `DataContext`.
  - **`SalesViewModel.cs`**: xoá toàn bộ state/logic panel — 5 property trạng thái (`IsReportPanelExpanded`/`IsReportLoading`/`HasReportError`/`ReportErrorMessage`/`HasReportRows`), 8 property filter (`SelectedEmployee`/`SelectedCustomer`/`SelectedUnit`/`SelectedCategory`/`SelectedPeriod`/`AreAllProductsSelected`/`FromDate`/`ToDate`), 5 collection (`Employees`/`Customers`/`Units`/`Categories`/`ProductItems`/`Periods`/`ReportRows`), field `_allProducts`/hằng `AllOption`, và toàn bộ method (`ToggleReportPanel`/`InitializeReportPanelAsync`/`LoadLookupsAsync`/`OnSelectedUnitChanged`/`OnSelectedCategoryChanged`/`OnSelectedPeriodChanged`/`OnAreAllProductsSelectedChanged`/`IsAnyOption`/`ApplyProductFilter`/`ApplyPeriodPreset`/`StartOfWeek`/`ClearReportFilters`/`LoadReportPanelAsync`/`DrillDownReportRow`). Constructor bỏ 4 dependency `IGetSalesOrderSummaryReportUseCase`/`IGetProductsUseCase`/`IGetEmployeesUseCase`/`IGetCustomersUseCase` — file giờ chỉ còn 6 command điều hướng/mở window (`GoBack`/`NavigateToHome`/`NavigateToSalesOrders`/`NavigateToSalesReturns`/`OpenDeposit`/`NavigateToDepositDeductionReport`) + `OpenReportCommand`.
  - Xoá hẳn `Domain/Models/ProductSalesSummaryRow.cs` — model chỉ phục vụ riêng panel này (`ReportRows`), không còn nơi nào tham chiếu sau khi gỡ panel (đã grep xác nhận trước khi xoá) — khác các file `SalesOrderReportView`/`SalesOrderReportFilterWindow` (giữ lại vì vẫn đang dùng qua tile, không phải dead code).
  - Không có BE change nào. WPF build 0 lỗi. Chưa test thật trên UTM.*

*Updated 2026-08-26 (làm đẹp lại 5 tile "Nghiệp vụ bán hàng" trên `SalesView`): theo yêu cầu — trước đó mỗi tile tự khai `Border.Style` riêng lẻ (trùng lặp 5 lần), icon emoji trơ trực tiếp trên nền trắng, hover chỉ đổi màu viền sang xám. Đổi sang:
  - 2 `Style` dùng chung trong `UserControl.Resources` — `SalesTile` (Border chính: `220x66`, bo góc `AppRadius.Large`, `DropShadowEffect` nhẹ thường trực để tạo cảm giác nổi khối, hover đổi viền sang `AppColor.BorderActive` (cam brand) + bóng đậm/rộng hơn hẳn thay vì chỉ đổi viền xám) và `SalesTileIconBadge` (khung tròn `40x40`, nền `AppColor.BackgroundBrand` — cam nhạt có sẵn trong palette, không thêm màu mới) — mỗi tile giờ chỉ còn `Style="{StaticResource SalesTile}"` thay vì khai lại y hệt 5 lần.
  - Icon đặt trong khung tròn màu thay vì trơ trên nền trắng; label đổi `FontWeight="SemiBold"` cho rõ hơn; label mục "Nghiệp vụ bán hàng" cũng đổi `SemiBold`.
  - Chỉ dùng đúng token màu có sẵn (`AppColor.BackgroundBrand`/`BorderActive`) — không đưa thêm hue mới (giữ nhất quán với phần còn lại của app chỉ dùng 1 tông cam brand làm accent).
  - Không đổi command/binding/logic nào. WPF build 0 lỗi. Chưa test thật trên UTM.*

*Updated 2026-08-28 (menu chuột phải trên `SalesOrderListView` + tính năng "Nhân bản" + cột "Diễn giải"/"Tiền thuế GTGT"): theo yêu cầu thêm right-click context menu (Thêm/Nhân bản/Xem/Xóa/Gửi email,Zalo) trên grid "Chứng từ bán hàng".
  - **`SalesOrderListView.xaml.cs`**: `OrdersGrid_PreviewMouseRightButtonDown` — tự chọn dòng nằm dưới con trỏ chuột (qua `FindAncestor<DataGridRow>` helper mới tìm ancestor từ `e.OriginalSource`) TRƯỚC khi context menu mở, để right-click 1 dòng chưa phải dòng đang chọn vẫn tác động đúng dòng đó thay vì dòng cũ.
  - **`SalesOrderListView.xaml`**: `ContextMenu` mới trên `DataGrid` — "➕ Thêm" (`AddSalesOrderCommand` có sẵn), "📋 Nhân bản" (mới), "👁 Xem" (mở read-only), "🗑 Xóa" (`DeleteSalesOrderCommand` có sẵn), submenu "📧 Gửi" chứa "Email"/"Zalo" (2 command mới).
  - **`SalesOrderListViewModel.cs`**: `DuplicateSalesOrderCommand` gọi `ISalesOrderService.DuplicateAsync(id)` (typed HttpClient → `POST /api/v1/sales-orders/{id}/duplicate`, xem `be-window-lamour/.../Sales/docs/sales.md`) rồi `LoadSalesOrdersAsync()` reload list — **bắt buộc capture `sourceId`/`sourceDocumentNumber` làm local var TRƯỚC khi gọi `LoadSalesOrdersAsync()`**: reload xoá sạch `SalesOrders` rồi add lại, việc này tự động reset `SelectedOrder` về `null` (do DataGrid `SelectedItem` TwoWay binding) — đọc `SelectedOrder.DocumentNumber` SAU reload để hiện message "Nhân bản thành công" sẽ `NullReferenceException` dù bản sao đã tạo thành công ở BE (bug đã gặp + fix trong lần build này — xác nhận qua chứng từ mới đã có trong DB dù message báo lỗi). `SendEmailCommand`/`SendZaloCommand` — tái dùng `BuildSingleOrderWorkbook` (export 1 chứng từ ra Excel, logic sẵn có từ tính năng xuất Excel) + `ReportSharingHelper` (mở app Email/Zalo với file đính kèm, cũng đã có sẵn dùng cho báo cáo khác) — không viết thêm luồng chia sẻ mới, chỉ nối 2 hạ tầng có sẵn lại cho 1 chứng từ đơn lẻ.
  - **`SalesOrderListItem.cs`**: thêm `Description` (Diễn giải), `TotalTax` (Tiền thuế GTGT) — cả 2 field đã có sẵn trên `SalesOrderResponseDto` (`Description`, `TotalTaxAmount`), chỉ trước đó chưa map lên model hiển thị của list. `SalesOrderListViewModel` thêm `TotalTaxSum` (tổng cột, hiện ở footer) — cùng công thức `Sum` như "Tổng tiền hàng" hiện có.
  - Không có BE change nào (endpoint Duplicate xem BE doc riêng). WPF build 0 lỗi.*

*Updated 2026-08-28 (chốt cuối cùng độ rộng cột hóa đơn in `SalesOrderPrintWindow`, sau nhiều vòng chỉnh vì bug wrap chữ): trải qua 4 vòng chỉnh liên tiếp — hẹp quá thì STT/Số lượng 3 chữ số bị wrap xuống dòng, TÊN SẢN PHẨM hẹp quá thì tên dài bị wrap gây lệch hàng với các cột số bên cạnh. Chốt cuối: `ProductTableColumnWidths = { 26, 84, 26, 62, 42, 82, 84, 84 }` (STT/Tên SP/ĐVT/SL/Đơn giá/Thành tiền/Thuế suất/Tổng cộng) + `HeaderFontSize = 10`; dòng tổng cộng tách thành 2 cell riêng (label + value) thay vì 1 cell gộp để căn đúng cột. **Không chỉnh lại 3 giá trị THÀNH TIỀN/THUẾ SUẤT/TỔNG CỘNG nữa** (comment cảnh báo đã thêm sẵn trong code) — đã tốn nhiều vòng lặp để tới được set số ổn định này, chỉnh lệch dù nhỏ dễ gây quay lại đúng bug wrap đã fix. WPF build 0 lỗi.*

*Updated 2026-08-31 (đổi mặc định lọc ngày trên "Chứng từ bán hàng" sang "Đầu tháng đến hiện tại"): `SalesOrderListViewModel` trước đây mặc định `FilterFromDate`/`FilterToDate` = HÔM NAY (chủ đích, comment cũ ghi rõ "giống MISA") — đổi sang đầu-tháng-đến-hôm-nay, cùng đợt đồng bộ toàn app (đã hỏi lại qua `AskUserQuestion` vì đây là quyết định có chủ đích trước đó, không phải thiếu sót — user xác nhận đổi luôn cho đồng bộ). Không có Kỳ dropdown ở màn này (chỉ 2 `DatePicker` Từ/Đến ngày) nên chỉ đổi field initializer, không thêm UI mới. Xem `SalesReturn/docs/sales-return.md` mục 6 để biết danh sách đầy đủ các màn khác cũng đổi theo cùng đợt. Không có BE change. WPF build 0 lỗi.*
