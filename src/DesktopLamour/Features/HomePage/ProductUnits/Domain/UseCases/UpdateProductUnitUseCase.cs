...
public sealed class UpdateProductUnitUseCase : IUpdateProductUnitUseCase
{
    private readonly IProductUnitRepository _repository;
    public UpdateProductUnitUseCase(IProductUnitRepository repository) => _repository = repository;

    public async Task<ProductUnit> ExecuteAsync(UpdateProductUnitInput input, CancellationToken ct = default)
    {
        ...
        if (existing.Any(u => u.Id != input.Id && u.Name.Equals(input.Name.Trim(), StringComparison.OrdinalIgnoreCase)))
